    ...
    def self.build(data)
      ...
      data.each do |hash|  
        if hash[:data].is_a? Array
          klass = "Absa::Esd::Transmission::#{hash[:type].capitalize.camelize}".constantize
          set.records << klass.build(hash[:data])
        else
          klass = "Absa::Esd::Transmission::#{self.partial_class_name}::#{hash[:type].capitalize.camelize}".constantize
